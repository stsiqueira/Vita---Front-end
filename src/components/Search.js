...
const Search = () => {
    ...
    const checkItemsByText = (foodName) =>{

        let itemExists = false;

        if (siteContent[0].Fruits[0].find(f => f.name === foodName)) {
            itemExists = true;
            SetItemtype("Fruits")
        }
        else if (siteContent[0].Vegetables[0].find(v => v.name === foodName)) {
            itemExists = true;
    ...
    function autocomplete(inp, arr) {
        ...
        function addActive(x) {
            ...
            removeActive(x);
            if (!currentFocus || currentFocus >= x.length)
                currentFocus = 0;
            if (currentFocus < 0)
    ...
    const searchExists = (e) => {
        ...
        checkItemsByText(e.target.value);
    }
